...
func (keycloakClient *KeycloakClient) NewAuthenticationExecution(execution *AuthenticationExecution) error {
	_, location, err := keycloakClient.post(fmt.Sprintf("/realms/%s/authentication/flows/%s/executions/execution", execution.RealmId, execution.ParentFlowAlias), &authenticationExecutionCreate{Provider: execution.Provider})
	if err != nil {
		return err
	}
...
func (keycloakClient *KeycloakClient) ListAuthenticationExecutions(realmId, parentAlias string) (AuthenticationExecutionList, error) {
	...
	err := keycloakClient.get(fmt.Sprintf("/realms/%s/authentication/flows/%s/executions", realmId, parentAlias), &authenticationExecutions, nil)
	if err != nil {
		return nil, err
	}
